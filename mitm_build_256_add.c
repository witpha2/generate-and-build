/*
   mitm_build_256_add.c  —  v16 (FIXED CSV verification + last step info) [ADD MODE]
   ✅ เปลี่ยนจาก SUB เป็น ADD (pubkey += step*G)
*/
...
int main(int argc, char *argv[]) {
    printf("🔨 mitm_build_256_add  v16  (ADD MODE: pubkey += step*G)\n");
    printf("==================================================================\n\n");
    ...
    // ========== CSV VERIFICATION (ADD MODE) ==========
    printf("================================================================================\n");
    printf("CSV VERIFICATION (iter, pubkey, accumulated_sum = hex_G_add)\n");  /* ✅ G_add */
    printf("================================================================================\n");
    printf("iter,pubkey,hex_G_add\n");  /* ✅ G_add */
    ...
    for (int wi = 0; wi < n_walkers && wi < 1; wi++) {
        ...
        for (int s = 1; s <= 4; s++) {
            ...
            /* ✅ เปลี่ยนจาก pub_tweak_sub_step เป็น pub_tweak_add_step */
            if (!pub_tweak_add_step(ctx, &pk, step)) break;
...
#endif
    {
        ...
        for (int wi = 0; wi < n_walkers; wi++) {
            ...
            for (uint64_t s = 0; s < steps_pw && !g_stop; s++) {
                ...
                /* ✅ เปลี่ยนจาก pub_tweak_sub_step เป็น pub_tweak_add_step */
                if (!pub_tweak_add_step(ctx, &pk, step)) {
                    fprintf(stderr, "\n⚠️ tweak_add failed at wi=%d s=%llu\n",
